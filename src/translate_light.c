...
/*   Updated: 2017/05/28 15:11:10 by thou             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	translate_light_1(int key, t_vector r, t_vector n, t_env *e)
{
	...
	if (key == KEY_W)
		e->light[0]->pos = (t_vector){light.pos.x + r.x * 10.0,
			light.pos.y + r.y * 10.0, light.pos.z + r.z * 10.0};
	else if (key == KEY_Q)
		e->light[0]->pos = (t_vector){light.pos.x - r.x * 10.0,
			light.pos.y - r.y * 10.0, light.pos.z - r.z * 10.0};
	else if (key == KEY_E)
		e->light[0]->pos = (t_vector){light.pos.x + e->up.x * 10.0,
			light.pos.y + e->up.y * 10.0, light.pos.z + e->up.z * 10.0};
	else if (key == KEY_D)
		e->light[0]->pos = (t_vector){light.pos.x - e->up.x * 10.0,
			light.pos.y - e->up.y * 10.0, light.pos.z - e->up.z * 10.0};
	else if (key == KEY_A)
		e->light[0]->pos = (t_vector){light.pos.x + n.x * 10.0,
			light.pos.y + n.y * 10.0, light.pos.z + n.z * 10.0};
	else if (key == KEY_S)
		e->light[0]->pos = (t_vector){light.pos.x - n.x * 10.0,
			light.pos.y - n.y * 10.0, light.pos.z - n.z * 10.0};
}
